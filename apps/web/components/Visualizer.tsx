...
function NeuronInstances({ positions, count, nodes }: { positions: Float32Array, count: number, nodes?: { type: string }[] }) {
...
function SynapseInstances({ positions, edges, onEdgeClick }: { positions: Float32Array, edges: GraphEdge[], onEdgeClick?: (i: number) => void }) {
    ...
    return (
        ...
        >
            <lineBasicMaterial color="#475569" transparent opacity={0.3} linewidth={1} />
        </lineSegments>
    );
}
...
export default function Visualizer({ data, onEdgeClick }: VisualizerProps) {
    ...
    const { positions, edges, count, nodes } = useMemo(() => {
        if (!data || data.nodeCount === 0) return { positions: new Float32Array(0), edges: [], count: 0, nodes: [] };
        
        const positions = computeLayout(data.nodeCount, data.edges, 50);
        return { positions, edges: data.edges, count: data.nodeCount, nodes: data.nodes };
    }, [data]);
    
    return (
        <div className="w-full h-full min-h-[500px] bg-black rounded-xl overflow-hidden relative">
            <Canvas camera={{ position: [0, 0, 40], fov: 50 }} raycaster={{ params: { Line: { threshold: 0.5 } } as any }}>
                ...
                {count > 0 && <NeuronInstances positions={positions} count={count} nodes={nodes} />}
